...
def main():
    parser = argparse.ArgumentParser(
        description="基于 rich 的多线程文件下载器，支持进度条和断点续传"
    )
    parser.add_argument("url", help="要下载的文件 URL")
    parser.add_argument("-o", "--output", help="保存的文件名（若省略则从 URL 提取）")
    parser.add_argument("-t", "--threads", type=int, default=4, help="并发线程数（默认 4）")
    parser.add_argument(
        "-r", "--resume", action="store_true", help="尝试断点续传（基于临时分片）"
    )
    ...
    if args.threads > 1:
        ...
        # 单线程
        success = download_single(args.url, output_path, args.resume)
