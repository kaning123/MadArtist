...
import numpy as np
import copy
ROOT_DIR = fl.get_parent_dir(fl.get_my_dir(),7)
import sys
...
class Main:
    ...
    def generate(self, txt: str, note: str | int | float | list) -> pathlib.Path:
        """
        ...
        # 从设置中读取语音（若无则使用默认）
        self.voice = settings.get("voice", asyncio.run(get_voice(txt)))["ShortName"]
        if not self.voice:
    ...
    async def _generate_async(self, 
        ...
        """异步执行 TTS 并保存文件"""
        if pitch == "+0hz":
            communicate = edge_tts.Communicate(txt, voice, rate=rate, volume=volume)
        else:
    ...
    def _parse_note_to_pitch(self, note):
        ...
        if isinstance(note, (int, float)):
            ...
            return f"{int(offset):+}Hz"  # 例如 "+10Hz" 或 "-10Hz"
        elif isinstance(note, str):
            try:
                freq = librosa.note_to_hz(note)
                offset = freq - self.base  # 参考频率
                return f"{int(offset):+}Hz"
            except Exception:
            ...
            note = support.note_list_mean(note)
            return self._parse_note_to_pitch(note)
        else:
            print(f"note: {note}")
