...
def User_ConfigEdit():
    config_path = os.path.join(get_my_dir(), "Config", "Settings.json")
    if not file_exists(config_path):
        create_dir(Path(config_path).parent)
        try:
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(DEFAULT_SAMPLE_JSON, f, indent=4, ensure_ascii=False)
        except Exception as e:
    ...
    try:
        ...
        config = DEFAULT_SAMPLE_JSON

    win = JsonEditorWindow(
        "MadArtist BaseVoice Generator Setting - Version Alpha_v0.0.2_202607",
        config
    )
    result = win.exec()

    if result == QDialog.DialogCode.Accepted:
        new_config = win.to_dict()
        try:
            with open(config_path, "w", encoding="utf-8") as f:
